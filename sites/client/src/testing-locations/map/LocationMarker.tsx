...
export const LocationMarker = ({ location: covidTestingLocation }: ILocationMarkerProps) => {
  ...
  const { id, site, location, currentWaitTime } = covidTestingLocation.facility;
      ...
      {(isHovered || selectedLocation?.facility.id === id) && (
        ...
        >
          <Paper elevation={3} className={classes.overlay}>
            ...
            <Typography variant="body2">Estimated wait time: {getTimeLabel(currentWaitTime)}</Typography>
          </Paper>
        </OverlayView>
      )}
    </>
